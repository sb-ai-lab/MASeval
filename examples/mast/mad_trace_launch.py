...
async def main(mad_traces, enable_tracing):
        ...
        if eval_input.agents_errors != None:
            ...
            if enable_tracing:
                with judge_client.start_as_current_span(
                    ...
                    output_dir = Path(__file__).parent / "mast_results"
                    output_dir.mkdir(exist_ok=True)
                    output_file = (
                        output_dir / f"evaluation_results_{task['trace_id']}.json"
                    )
                ...
                output_dir = Path(__file__).parent / "mast_results"
                output_dir.mkdir(exist_ok=True)
                output_file = output_dir / f"evaluation_results_{task['trace_id']}.json"
...
if __name__ == "__main__":
    asyncio.run(main(mad_traces="data/mad_dataset.json", enable_tracing=True))
