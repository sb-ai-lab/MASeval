...
def _extract_ordered_agents_from_graph(graph: str | None) -> list[str]:
    """
    Extract ordered agent names from a Mermaid graph string.
    
    Args:
        graph: Mermaid graph string (e.g., "graph LR\n2b448c2d[PlanningAgent] --> ...")
    
    Returns:
        list[str]: Ordered list of agent names (topological order if graph has dependencies,
                   otherwise order of appearance)
    """
    if not graph or not isinstance(graph, str):
        return []
    
    # Pattern to match node definitions: node_id[AgentName]
    # Matches patterns like: 2b448c2d[PlanningAgent] or abc123[AgentName]
    node_pattern = r'(\w+)\[([^\]]+)\]'
    
    # Find all node definitions
    nodes = {}
    for match in re.finditer(node_pattern, graph):
        node_id = match.group(1)
        agent_name = match.group(2)
        nodes[node_id] = agent_name
    
    if not nodes:
        return []
    
    # Build dependency graph and find topological order
    # Pattern to match edges: source_id[SourceName] --> target_id[TargetName]
    ...
    for match in re.finditer(edge_pattern, graph):
        source_id = match.group(1)
        target_id = match.group(3)
        
        if source_id in nodes and target_id in nodes:
            source_agent = nodes[source_id]
            target_agent = nodes[target_id]
            all_agent_names.add(source_agent)
            all_agent_names.add(target_agent)
            
            graph_edges[source_agent].append(target_agent)
            in_degree[target_agent] += 1
            if source_agent not in in_degree:
                in_degree[source_agent] = 0
    
    # Add nodes that don't have edges
    for agent_name in nodes.values():
        if agent_name not in in_degree:
    ...
    # Add any remaining agents (in case of cycles or disconnected nodes)
    remaining = all_agent_names - set(ordered_agents)
    if remaining:
        # Add them in the order they appear in the graph
        seen_in_graph = []
        for match in re.finditer(node_pattern, graph):
            agent_name = match.group(2)
            if agent_name in remaining and agent_name not in seen_in_graph:
                seen_in_graph.append(agent_name)
        ordered_agents.extend(seen_in_graph)
...
def parse_langfuse_task(trace: ObservationsView) -> EvaluationInput:
    ...
    # Extract ordered agent names from mermaid_graph or graph
    ordered_agent_names = _extract_ordered_agents_from_graph(agents_pool.graph)
    
    # Reorder agents_pool.agents to match the order from the graph
    if ordered_agent_names:
        # Create a dictionary for quick lookup of agent descriptions
        agents_dict = {agent.agent_name: agent for agent in agents_pool.agents}
        
        # Reorder agents based on graph order
        reordered_agents = []
        for agent_name in ordered_agent_names:
            if agent_name in agents_dict:
                reordered_agents.append(agents_dict[agent_name])
        
        # Add any agents not in the graph (shouldn't happen, but just in case)
        seen_names = set(ordered_agent_names)
        for agent in agents_pool.agents:
            if agent.agent_name not in seen_names:
                reordered_agents.append(agent)
        
        # Update agents_pool with reordered agents
        agents_pool = AgentsPool(
            agents=reordered_agents,
            graph=agents_pool.graph
        )
        agents = ordered_agent_names
    else:
        # Fallback: if graph extraction fails, use agents from pool in their original order
        agents = [agent.agent_name for agent in agents_pool.agents]
...
def _get_agents_pool(observations: list[ObservationsView]) -> list[AgentsPool]:
    ...
    # First, get mermaid_graph from output (same way as before)
    for obs in observations:
        obs_name = getattr(obs, "name", None)
        ...
        if isinstance(output, dict):
            graph = output.get("mermaid_graph")
            if graph:
                break

    # Then, extract agent descriptions from observations ending with " run" (excluding GraphGenerator)
    for obs in observations:
        obs_name = getattr(obs, "name", None)
        if not (isinstance(obs_name, str) and obs_name.endswith(" run")):
            continue
        
        # Exclude GraphGenerator run
        if obs_name == "GraphGenerator run":
            continue

        # Extract agent name from observation name
        agent_name = obs_name.replace(" run", "")
        if not agent_name:
            continue

        # Get instructions from metadata attributes (as shown in notebook)
        metadata = getattr(obs, "metadata", {}) or {}
        attributes = (
            metadata.get("attributes", {}) if isinstance(metadata, dict) else {}
        )
        
        system_instructions_raw = attributes.get("gen_ai.system_instructions")
        instructions = ""
        
        if system_instructions_raw:
            try:
                system_instructions = json.loads(system_instructions_raw)
                if isinstance(system_instructions, list) and len(system_instructions) > 0:
                    # Extract content from first element as shown in notebook
                    instructions = system_instructions[0].get("content", "")
            except (TypeError, json.JSONDecodeError) as e:
                logger.error(f"Error parsing system instructions for {obs_name}: {e}")

        # Add agent description if not already present
        if not any(desc.agent_name == agent_name for desc in agents_descriptions):
    ...
    agents_pool = AgentsPool(
        agents=agents_descriptions, graph=graph or ""
    )
...
if __name__ == "__main__":
    ...
    trace_data = lf.api.trace.get(
        ...
        # "4e29df5dba14fd3652aba9708af5c38a",
        "e2e8ac400a5a0097cc1d2aee6d42c020"
